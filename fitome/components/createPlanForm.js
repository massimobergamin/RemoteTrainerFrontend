...
import { getClients, getWorkout, postPlan } from '../redux/trainer';
...
const CreatePlanForm = () => {
  ...
  const { clients, workouts } = useSelector(state => state.trainer);
  ...
  const setReps = (e, index) => {
    insertAt(detailState.reps, index, e.currentTarget.value);
  }

  const setSets = (e, index) => {
    insertAt(detailState.sets, index, e.currentTarget.value);
  }
...
const showExercises = (exercises, reps, sets, notes) => {
    ...
    return planState.details.map((workout,index) => {
      return (
        <div className="plan_card">
          ...
          {showExercises(workout.exercises, workout.reps, workout.sets, workout.trainer_notes)}
        </div>
      );
    });
  };
      ...
      <form className="fullFormContainer">
        ...
        <label>End Date:</label>
           <input className="createPlanInput" type="date" min={getDate(moment(planState.start_date).add(1, 'days'))} disabled={planState.start_date === '' || planState.details.length} name="end date" value={planState.end_date}
            onChange={(e) => {
             if(e.target.value > planState.start_date)
              setPlanState({ ...planState, end_date:e.target.value });
